...
장점: 검색 속도 향상, 정렬 및 그룹화 작업 효율 증가

단점: 저장 공간 추가 소모, 데이터 수정 시 인덱스도 갱신되어 성능 저하 가능
**18 인덱스를 사용하는 것이 바람직한 경우와 그렇지 않은 경우를 구분하여 설명하시오.**
바람직한 경우: 자주 조회되는 컬럼, WHERE 절에 자주 사용되는 컬럼

바람직하지 않은 경우: 데이터 변경이 매우 빈번한 컬럼, 소량의 데이터 테이블
