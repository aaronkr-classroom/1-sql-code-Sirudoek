...
2. 데이터베이스 관리자인 DBA는 (Database Adminsistrator)의 약어이다.
...
6. 데이터베이스와 DBMS 의 종류를 분류하는 가장 중요한 기준은 바로 (데이터모)이다.
7. (개념스키)는 모든 데아터베이스 사용자들의 관점을 통합하여 조직이나 기관의 데이터베이스 전체를 명세한 데이터베이스 구조아다.
...
- 1세대 (1960년대 ~ 1970년대)는 계층형과 네트워크형 구조로, 대형 컴퓨터에서 주로 사용됨
- 2세대 (1980년대 초반 ~ 현재)는 관계형 데이터베이스로 데이터를 테이블 형태로 구성
- 3세대 (1980년대 후반 ~ 현재)는 객체 지향형과 객체 관계형으로, 객체 지향 언어의 상속 개념을 도입하고 RDBMS와 결합한 구조로 발전
- 4세대 (2010년대 중반 ~ 현재)는 빅데이터와 비정형 데이터를 처리하는 NoSQLDBMS로, 스키마-리스 구조와 고성능, 확장성을 특징으로 함

**19 데이터 사전(data dictionary)과 데이터 디렉토리 (data directory)의 차이점을 설명하시오.**

- 데이터 사전은 데이터베이스 정의, 스키마 간의 매핑 정보, 제약 조건 등을 저장하는 시스템 카탈로그
  - 데이터 디렉토리는 데이터의 위치 정보를 저장하는 시스템 데이터베이스 
...
- 외부 스키마: 데이터베이스 사용자나 응용 프로그램이 데이터를 어떻게 볼지에 대한 논리적 뷰를 제공
